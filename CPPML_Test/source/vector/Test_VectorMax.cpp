...
struct VectorMax : ::testing::TestWithParam<std::tuple<
    int,   // length
...
TEST_P(VectorMax, Test_VectorMax)
{
    ...
    // run test code
    ref.Start();
    test::TestVectorMax(src, dst_ref);
    ref.End();

    // run lib code
    lib.Start();
    auto status = src.Max(dst_lib, pool);
    lib.End();
