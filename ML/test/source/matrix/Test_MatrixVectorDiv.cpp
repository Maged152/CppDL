...
void TestMatrixVectorDiv_Row(const Matrix& src1, const Vector& src2, Matrix& dst)
{
...
void test::Test_MatrixVectorDiv(std::vector<int>& mat_rows, std::vector<int>& mat_cols, float utilization, float threshold, float min, float max)
{
	HANDLE col_handle;
	...
	for (int r = 0; r < mat_rows.size(); r++)
	{
		for (int c = 0; c < mat_cols.size(); c++)
		{
			...
			// div matrix function
			opt.Start();
			auto status = src1.Div(src2, dst_opt, broad_cast, utilization);
			opt.End();
			// compare the results
