...
void TestMatrixSub(const Matrix& src1, const float val, Matrix& dst)
{
...
void test::Test_MatrixScalarSub(std::vector<int>& mat_rows, std::vector<int>& mat_cols, int num_threads, float threshold, float min, float max)
{
	HANDLE col_handle;
	...
	for (int r = 0; r < mat_rows.size(); r++)
	{
		for (int c = 0; c < mat_cols.size(); c++)
		{
			...
			ref.Start();
			TestMatrixSub(src1, val, dst_ref);
			ref.End();
			// add matrix function
			opt.Start();
			auto status = src1.Sub(val, dst_opt, pool);
			opt.End();
			// compare the results
