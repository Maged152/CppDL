...
void test::Test_MatrixAdd(std::vector<int>& mat_rows, std::vector<int>& mat_cols, float threshold, float min, float max)
{
	HANDLE col_handle;
	...
	for (int r = 0; r < mat_rows.size(); r++)
	{
		for (int c = 0; c < mat_cols.size(); c++)
		{
			...
			// add matrix function
			opt.Start();
			auto status = src1.MatrixAdd(src2, dst_opt, 1.0f);
			opt.End();
			// compare the results
