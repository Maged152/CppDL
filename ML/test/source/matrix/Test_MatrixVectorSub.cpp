...
void test::Test_MatrixVectorSub(std::vector<int>& mat_rows, std::vector<int>& mat_cols, unsigned int  num_threads, float threshold, float min, float max)
{
	HANDLE col_handle;
	...
	for (int r = 0; r < mat_rows.size(); r++)
	{
		for (int c = 0; c < mat_cols.size(); c++)
		{
			...
			// sub matrix function
			opt.Start();
			auto status = src1.Sub(src2, dst_opt, broad_cast, pool);
			opt.End();
			// compare the results
