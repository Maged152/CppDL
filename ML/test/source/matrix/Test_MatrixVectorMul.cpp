...
void TestMatrixVectorMul_Row(const Matrix& src1, const Vector& src2, Matrix& dst)
{
...
void TestMatrixVectorMul_Coloumn(const Matrix& src1, const Vector& src2, Matrix& dst)
{
	for (int r = 0; r < src1.Rows(); r++)
	{
		for (int c = 0; c < src1.Columns(); c++)
...
void test::Test_MatrixVectorMul(std::vector<int>& mat_rows, std::vector<int>& mat_cols, float utilization, float threshold, float min, float max)
{
	HANDLE col_handle;
	...
	for (int r = 0; r < mat_rows.size(); r++)
	{
		for (int c = 0; c < mat_cols.size(); c++)
		{
			...
			{
				ref.Start();
				TestMatrixVectorMul_Row(src1, src2, dst_ref);
				ref.End();
			}
			else
			{
				ref.Start();
				TestMatrixVectorMul_Coloumn(src1, src2, dst_ref);
				ref.End();
			}
			// mul matrix function
			opt.Start();
			auto status = src1.Mul(src2, dst_opt, broad_cast, utilization);
			opt.End();
			// compare the results
