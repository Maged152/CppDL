...
void TestVectorScalarMul(const Vector& src1, const float val, Vector& dst)
{
...
void test::Test_VectorScalarMul(std::vector<int>& vec_len, float utilization, float threshold, float min, float max)
{
	HANDLE col_handle;
	...
	for (int l = 0; l < vec_len.size(); l++)
	{
		...
		// test Vector Mul
		ref.Start();
		TestVectorScalarMul(src1, val, dst_ref);
		ref.End();
		// Mul Vector function
		opt.Start();
		auto status = src1.Mul(val, dst_opt, utilization);
		opt.End();
		// compare the results
