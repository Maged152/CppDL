...
bool test::Test_VectorNorm(std::vector<int>& vec_len, unsigned int  num_threads, float threshold, float min, float max)
{
	HANDLE col_handle;
	...
	for (int l = 0; l < vec_len.size(); l++)
	{
		...
		opt.Start();
		auto status = src.Norm(norm, dst_opt, pool);
		opt.End();
		// compare the results
