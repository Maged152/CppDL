...
bool test::Test_VectorMax(std::vector<int>& vec_len, unsigned int  num_threads, float threshold, float min, float max)
{
	HANDLE col_handle;
	...
	for (int l = 0; l < vec_len.size(); l++)
	{
		...
		// test code
		ref.Start();
		TestVectorMax(src, dst_ref);
		ref.End();
		// multi-threading code
		opt.Start();
		auto status = src.Max(dst_opt, pool);
		opt.End();
		// compare the results
