...
bool test::Test_VectorMean(std::vector<int>& vec_len, unsigned int  num_threads, float threshold, float min, float max)
{
	HANDLE col_handle;
	...
	for (int l = 0; l < vec_len.size(); l++)
	{
		...
		// test code
		ref.Start();
		TestVectorMean(src, dst_ref);
		ref.End();
		// multi-threading code
		opt.Start();
		auto status = src.Mean(dst_opt, pool);
		opt.End();
		// compare the results
