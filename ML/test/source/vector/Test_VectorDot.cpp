...
bool test::Test_VectorDot(std::vector<int>& vec_len, unsigned int  num_threads, float threshold, float min, float max)
{
	HANDLE col_handle;
	...
	{	
		...
		float current_threshold = threshold * std::pow(len, 5.0f/3);
		...
		if (!n_res)
		{
			...
			std::cout << "diff = " << dst_ref - dst_opt << "  ,threshold = "<< current_threshold <<  "\n";
			num_failed_cases++;
		}
