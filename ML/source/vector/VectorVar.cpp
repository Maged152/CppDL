...
namespace qlm
{
	Status Vector::Var(float& dst, ThreadPool& pool) const
	{
		...
		auto sum_op = [](const float* const __restrict  src, const float mean,
...
#pragma omp unroll full
		...
		{
			futures[i] = pool.Submit(sum_op, &data[next_idx], mean, thread_length + 1);
...
#pragma omp unroll full
		...
		{
			futures[i] = pool.Submit(sum_op, &data[next_idx], mean, thread_length);
