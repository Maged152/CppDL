...
struct VectorVar : ::testing::TestWithParam<std::tuple<
    int,   // length
...
TEST_P(VectorVar, Test_VectorVar)
{
    // extract the parameters
    auto& [length, num_threads, min_val, max_val] = GetParam();

    // threshold
    const float percentage_threshold = 0.5f;
    ...
    // run lib code
    lib.Start();
    auto status = src.Var(dst_lib, pool);
    lib.End();
