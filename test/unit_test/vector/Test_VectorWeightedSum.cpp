...
struct VectorWeightedSum : ::testing::TestWithParam<std::tuple<
    int,   // length
...
TEST_P(VectorWeightedSum, Test_VectorWeightedSum)
{
    // extract the parameters
    auto& [length, num_threads, min_val, max_val, bias] = GetParam();
    ...
    // run test code
    ref.Start();
    test::WeightedSum(src1, src2, bias, dst_ref);
    ref.End();

    // run lib code
    lib.Start();
    auto status = src1.WeightedSum(src2, bias, dst_lib, pool);
    lib.End();
...
INSTANTIATE_TEST_CASE_P(
    Test_VectorWeightedSum, VectorWeightedSum,
    ::testing::Combine(
