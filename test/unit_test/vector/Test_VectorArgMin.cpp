...
struct VectorArgMin : ::testing::TestWithParam<std::tuple<
    int,   // length
...
TEST_P(VectorArgMin, Test_VectorArgMin)
{
    ...
    // run test code
    ref.Start();
    test::TestVectorArgMin(src, dst_ref);
    ref.End();

    // run lib code
    lib.Start();
    auto status = src.ArgMin(dst_lib, pool);
    lib.End();
...
INSTANTIATE_TEST_CASE_P(
    Test_VectorArgMin, VectorArgMin,
    ::testing::Combine(
