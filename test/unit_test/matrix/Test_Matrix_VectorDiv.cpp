...
struct MatrixDiv : ::testing::TestWithParam<std::tuple<
    int,   // rows
	int,   // cols
...
TEST_P(MatrixDiv, Test_MatrixDiv)
{
    ...
    // run test code
    ref.Start();
    test::Div(src1, src2, dst_ref, broad_cast);
    ref.End();

    // run lib code
    lib.Start();
    auto status = src1.Div(src2, dst_lib, broad_cast, pool);
    lib.End();
