...
struct MatrixScalarSub : ::testing::TestWithParam<std::tuple<
    int,   // rows
	int,   // cols
...
TEST_P(MatrixScalarSub, Test_MatrixScalarSub)
{
    ...
    // run test code
    ref.Start();
    test::Sub(src, value, dst_ref);
    ref.End();

    // run lib code
    lib.Start();
    auto status = src.Sub(value, dst_lib, pool);
    lib.End();
...
INSTANTIATE_TEST_CASE_P(
    Test_MatrixScalarSub, MatrixScalarSub,
    ::testing::Combine(
