...
struct MatrixSub : ::testing::TestWithParam<std::tuple<
    int,   // rows
	int,   // cols
...
TEST_P(MatrixSub, Test_MatrixSub)
{
    ...
    // run test code
    ref.Start();
    test::Sub(src1, src2, dst_ref, broad_cast);
    ref.End();

    // run lib code
    lib.Start();
    auto status = src1.Sub(src2, dst_lib, broad_cast, pool);
    lib.End();
